...
MIDI_PATH = "Fantaisie-Impromptu_in_C_Minor__Chopin.mid"
AUDIO_PATH = "Fantaisie-Impromptu_in_C_Minor__Chopin.mp3"
FRAME_FOLDER = "frames"
...
# Optional: parse tempo changes
default_tempo = 500000  # microseconds per beat (120 BPM)
current_time = 0
note_times = []

for msg in midi:
    current_time += msg.time  # time is in seconds if using Mido with `pre-converted` file
    if msg.type == 'note_on' and msg.velocity > 0:
        note_times.append((msg.note, round(current_time, 2)))

from music21 import converter, note, chord, articulations
import os

def detect_accented_notes(mxl_file_path: str):
    accented = []

    score = converter.parse(mxl_file_path)

    # Remove all existing tempo marks
    for mm in score.flat.getElementsByClass(tempo.MetronomeMark):
        score.remove(mm)

    # Insert fixed tempo (e.g., 84 BPM)
    ...
    sm = score.secondsMap  # Music21 timing map (may not always be accurate)

    for part in score.parts:
        for el in part.recurse().getElementsByClass([note.Note, chord.Chord]):
            if not hasattr(el, "articulations"):
                continue

            for a in el.articulations:
                if isinstance(a, articulations.Accent):
                    # Try to get accurate time from secondsMap
                    time_info = next((entry for entry in sm if entry['element'] == el), None)
                    if time_info:
                        t = round(time_info['startTime'], 3)
                    else:
                        # Fallback: calculate manually using absolute offset and fixed tempo
                        abs_offset = el.getOffsetInHierarchy(score)
                        t = round(abs_offset * seconds_per_beat, 3)

                    if isinstance(el, note.Note):
                        accented.append((el.pitch.midi, t))
                        print(f"{el.nameWithOctave} | measure={el.measureNumber} beat={el.beat} time={t}")
                    elif isinstance(el, chord.Chord):
                        for p in el.pitches:
                            accented.append((p.midi, t))
                            print(f"{p.nameWithOctave} | measure={el.measureNumber} beat={el.beat} time={t}")

    return accented

def is_accented_note(midi_pitch, midi_time, accented_notes_list, tolerance=0.15):
    midi_time = round(midi_time, 2)  # ensures sync with rounded accent times
    for pitch, acc_time in accented_notes_list:
        if pitch == midi_pitch and abs(midi_time - acc_time) < tolerance:
            return True
    return False
...
def reveal_image_with_music(mode="render", full_song=False):
    ...
    score = converter.parse("Fant.musicxml")
    accented_notes_list = detect_accented_notes("Fant.musicxml")
    print(accented_notes_list)
    ...
    RANDOM_GROUP_SEED = 41  # Change this for different random orders
    ...
    while video_time < midi_total_time and not quit_flag:
        ...
        while event_idx < total_events and midi_events[event_idx][0] <= video_time:
            ...
            if msg.type == 'note_on' and msg.velocity > 0:
                ...
                if is_accented_note(msg.note, video_time, accented_notes_list):
                    print(f"🔥 ACCENT detected at {video_time:.2f}s for note {msg.note}")
                    # Gather all unrevealed, non-dark pixels for notes < 60 at a random circle position
                    ...
                    # Pick a new random center for the circle every time
                    h, w = image.shape[:2]
                    ...
                    # Sort by distance to the random center, add randomness for organic effect
                    choices.sort(key=lambda p: ((p[0] - center_y) ** 2 + (p[1] - center_x) ** 2) * (1 + 0.2 * np.random.rand()))
                    # Reveal up to REVEAL_STEP*2 pixels closest to the new center
                    for y, x in choices[:REVEAL_STEP * 2]:
